...
def book_borrow(request, book_id, username):
    if request.method == 'GET':
        ...
        # 创建借阅记录
        borrow_record = Borrow(user=uname, book=book, borrow_date=timezone.now(), return_date=timezone.now() + timedelta(days=14), is_return=False, is_overtime=False, is_renew=False, renew_date=None)
        borrow_record.save()
