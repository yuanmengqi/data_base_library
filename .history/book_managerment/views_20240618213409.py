...
def index(request):
    '''borrow_to_delete = Borrow.objects.get(id=7)
        # 删除记录
    ...
    if request.method == 'POST' and request.POST:
        ...
        if e:
            if e.password == password:
                ...
                # 在网页上显示用户借阅过的书
                #borrowed_books = Borrow.objects.filter(user=e, is_return=False)
                return render(request, 'user_home.html', {'user': e}) #, 'borrowed_books': borrowed_books
            else:
...
def book_search(request):
    if request.method == 'POST':
        ...
        try:
            ...
            username = LmsUser.objects.get(username__icontains=uname)
            '''context = {
                'context': book
            }
            '''
...
def book_borrow(request, book_id, username_in):
    if request.method == 'GET':
        book = models.Book.objects.get(id=book_id) #int()
         # 检查用户是否已经借阅了5本图书
        #uname = request.session.get('id')
        username = LmsUser.objects.get(username=username_in)#int()
        print(username)
        uname = username

        # 如果已经借阅了这本书，就不能再次借阅了
        if Borrow.objects.filter( book=book, is_return=False).exists(): #user=uname,
            return render(request, 'already_borrowed.html')
...
def book_return(request, book_id, username):
    if request.method == 'GET':
        #book_id = request.GET.get('book_id')
        #book = Book.objects.get(id=book_id)
        book = models.Book.objects.get(id=book_id)#int()
        username = LmsUser.objects.get(username=username)#int()
        
        # 查找用户借阅的记录
        borrow_record = Borrow.objects.get(user=username, book=book, is_return=False)
        ...
        # 检查是否可以解除用户的借书限制
        overdue_records = overdue.objects.filter(user=username, is_return=False, return_date__lt=timezone.now())
...
def drop_book(request, book_id):
    ...
    # 删除borrow表中有关这本书的记录
    models.Borrow.objects.filter(book_id=book_id).delete()
    return redirect('/book_list/')
...
def edit_book(request, book_id):
    if request.method == 'POST':
        ...
        edit_obj.save()
        # 更新borrow表中有关这本书的记录
        #models.Borrow.objects.filter(book_id=book_id).update(book_name=new_name)
        return redirect('/book_list/')
