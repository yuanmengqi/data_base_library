...
def book_borrow(request, book_id, username):
    if request.method == 'GET':
        ...
        uname = LmsUser.objects.get(username=int(username))
        if Borrow.objects.filter(user=username, is_return=False).count() >= 5:
            # 如果已经借阅了5本图书，拒绝借阅请求
        ...
        # 创建借阅记录
        borrow_record = Borrow(user=username, book=book, borrow_date=timezone.now(), return_date=timezone.now() + timedelta(days=14))
        borrow_record.save()
        
        # 更改用户的借书数量
        request.user.borrow_num += 1
        request.user.save()
