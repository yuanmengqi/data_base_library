...
def book_search(request, user_id):
    if request.method == 'POST':
        search_query = request.POST.get('search_query')
        try:
            book = Book.objects.get(name__icontains=search_query)
            return render(request, 'book_search_result.html', {"book":book}, {"user_id":user_id})
...
def book_borrow(request, book_id, uname):
    if request.method == 'GET':
        ...
        return redirect('book_search', uname)
    else:
        return render(request, 'book_search.html')
...
def book_return(request):
    if request.method == 'GET':
        ...
        return redirect('book_search', request.user)
    else:
        return render(request, 'book_search.html')
