...
def book_borrow(request):
    if request.method == 'GET':
        ...
        # 检查用户是否已经借阅了5本图书
        if BorrowRecord.objects.filter(user=request.user, return_date__isnull=True).count() >= 5:
            # 如果已经借阅了5本图书，拒绝借阅请求
            return render(request, 'borrow_limit_exceeded.html')
        
        # 检查用户是否有逾期未归还的图书
        overdue_record = BorrowRecord.objects.filter(user=request.user, return_date__isnull=True, due_date__lt=timezone.now())
        if overdue_record.exists():
            # 如果有逾期未归还的图书，设置用户的可借状态为no
            request.user.can_borrow = False
            request.user.save()
            return render(request, 'overdue_book.html')
        
        # 创建借阅记录
        borrow_record = BorrowRecord(user=request.user, book=book, borrow_date=timezone.now(), due_date=timezone.now() + timedelta(days=14))
        borrow_record.save()
