...
def book_search(request):
    if request.method == 'POST':
        ...
        try:
            ...
            '''
            return render(request, 'book_search_result.html', {"book":book})
...
def book_borrow(request, book_id):
    ...
    if request.method == 'GET':
        ...
        if int(book_id) == 12:
            ...
            book_id2 = int(book_id)
            print(book_id2)
            book = models.Book.objects.get(id=book_id2)
        ...
        #return HttpResponse("Invalid book ID", status=400)
         # 检查用户是否已经借阅了5本图书
        if Borrow.objects.filter(user=request.user.id, is_return=False).count() >= 5:
            # 如果已经借阅了5本图书，拒绝借阅请求
            return render(request, 'borrow_limit_exceeded.html')
        
        # 检查用户是否有逾期未归还的图书
        overdue_records = overdue.objects.filter(user=request.user.id, is_return=False, return_date__lt=timezone.now())
        ...
        # 创建借阅记录
        borrow_record = Borrow(user=request.user.id, book=book, borrow_date=timezone.now(), return_date=timezone.now() + timedelta(days=14))
        borrow_record.save()
